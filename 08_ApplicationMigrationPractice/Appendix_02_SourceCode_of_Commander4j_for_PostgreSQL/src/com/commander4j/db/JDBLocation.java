...
public class JDBLocation
{
	...
	public JDBLocation(String locationId, String plant, String warehouse, String gln, String description, String location, String type, String section, String bin, String equipmenttrackingid)
	{
	...
	public Vector<JDBLocation> getLocationData(PreparedStatement criteria) {
		...
		{
			result.addElement(new JDBLocation("location_id", "plant", "warehouse", "gln", "description", "location", "type", "section", "bin", "equipment"));
		}
		else
		{
			try
			{
				...
				while (rs.next())
				{
					result.addElement(new JDBLocation(rs.getString("location_id"), rs.getString("plant"), rs.getString("warehouse"), rs.getString("gln"), rs.getString("description"), rs.getString("storage_location"), rs.getString("storage_type"), rs
							.getString("storage_section"), rs.getString("storage_bin"), rs.getString("equipment_tracking_id")));
				}
				rs.close();
	...
	public boolean update() {
		...
		if (isValid() == true)
		{
			try
			{
				...
				stmtupdate.setString(19, getLocationID());
				stmtupdate.execute();
