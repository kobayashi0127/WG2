...
public class JDBMaterial
{
	...
	public JDBMaterial(String material, String description, String type, String baseUom, Integer shelflife, String shelflifeuom, String shelfliferule, String equipmentType)
	{
	...
	public Vector<JDBMaterial> getMaterialData(PreparedStatement criteria) {
		...
		{
			result.addElement(new JDBMaterial("material", "description", "type", "base_uom", 0, "shelf life uom", "rounding rule", "equipmentType"));
		}
		else
		{
			try
			{
				...
				while (rs.next())
				{
					result.addElement(new JDBMaterial(rs.getString("material"), rs.getString("description"), rs.getString("material_type"), rs.getString("base_uom"), rs.getInt("shelf_life"), rs.getString("shelf_life_uom"), rs
							.getString("shelf_life_rule"), rs.getString("equipment_type")));
				}
				rs.close();
	...
	public boolean update() {
		...
		if (isValid() == true)
		{
			try
			{
				...
				stmtupdate.setString(15, getMaterial());
